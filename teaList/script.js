...
class Tea{
  ...
  renderRow(){
    ...
    row.append(name,type,region,cost);
    return row;
  }
  ...
  renderDetails(){
    ...
    cardBody.append(cardTitle,p,cardText);

    card.append(cardImg,overlay,cardBody);
    return card;
  }
}
...
class TeaShop{
  constructor(teaData,domElement){
    this.teaDate = teaData;
    this.details = $(domElement.details);
    this.teaArea = $(domElement.teaArea)

    this.addTea = this.addTea.bind(this);
    this.handleClick = this.handleClick.bind(this);
    

    this.list = [];
    this.teaDate.forEach(this.addTea);
  }

  

  addTea(tea){
    this.list.push(new Tea(tea,{
      click:this.handleClick
    }))
  }
  displayAllTea(){
    var tea = this.list.map(v => {
      return v.renderRow();
    })

    this.teaArea.empty().append(tea);
  }

  handleClick(tea){
    var detail = tea.renderDetails();
    this.details.empty().append(detail);
  }
}

$(document).ready(initalized);

function initalized() {
  var teaShop = new TeaShop(TeaData,{
    details:".details-area",
    teaArea:".tea-area"
  })

  teaShop.displayAllTea();
}